...
async function ambilDataSheet() {
    ...
    try {
        const [resWilayah, resSppg, resKopdes, resIntel, resPidum, resDatun] = await Promise.all([
            dbClient.from('wilayah_induk').select('*'),
            dbClient.from('data_sppg').select('*'),
            dbClient.from('data_kopdes').select('*'),
            dbClient.from('data_intel').select('*'),
            dbClient.from('data_pidum').select('*'),
            dbClient.from('data_datun').select('*')
        ]);
        ...
        ui.formLogin.addEventListener('submit', async (e) => {
            ...
            try {
                const { data, error } = await dbClient.auth.signInWithPassword({
                    email: `${user}@nganjuk.go.id`,
                    password: pass
                });

                if (error) {
                    err.innerText = error.message || 'Login Gagal. Cek kembali akun Anda.';
...
// ==========================================
const clean = {
    kec: (n) => n ? String(n).replace(/^(kecamatan|kec\.|kec\s+)/i, '').trim().toUpperCase() : "",
...
function dapatkanDataTerfilter() {
    ...
    return dataSPPG.filter(s => {
        ...
        if (qDesa !== "") {
            ...
            matchDesa = sAlamat.includes(qDesa) || sDesa.includes(qDesa);
        }
    ...
    if (element) {
        element.addEventListener('change', (e) => {
            ...
            if (id === 'filter-kecamatan') {
                ...
                if (val !== "") {
                    let infoDesa = dataWilayah.find(w => w.level === 'desa' && clean.desa(w.nama_wilayah) === clean.desa(val) && clean.kec(w.kecamatan || nKec) === clean.kec(nKec));
                    if (!infoDesa) infoDesa = dataWilayah.find(w => w.level === 'desa' && clean.desa(w.nama_wilayah) === clean.desa(val));
...
function updateTampilanGlobal() {
    ...
    animateValue('stat-kopdes', terfilter.filter(s => s.topik === 'kopdes').length);
...
function renderRecentActivities() {
    ...
    if (dataKegiatan.length === 0) {
        container.innerHTML = '<tr><td colspan="4" class="py-12 text-center text-xs text-gray-400 italic bg-gray-50/20">Belum ada data kegiatan terdaftar.</td></tr>';
        return;
    }
...
function updateDetailPanel(data, nKec, nDesa = null) {
    ...
    const el = {
        ...
        k: document.getElementById('pemimpin-kontak'),
        f: document.getElementById('pemimpin-foto')
    };

    if (data && data.nama_pemimpin) {
        const jab = data.level === 'kecamatan' ? 'Camat' : 'Lurah';
        if (el.w) el.w.innerText = `${label} ${data.nama_wilayah}`;
        if (el.p) el.p.innerText = data.nama_pemimpin;
        if (el.k) el.k.innerText = `Jabatan: ${jab} | WA: ${data.kontak_pemimpin}`;
        if (el.f) el.f.src = data.foto_pemimpin || `https://ui-avatars.com/api/?name=${data.nama_pemimpin}&background=00ac69&color=fff`;
    } else {
...
function muatDaftarTabel(list) {
    ...
    const buatTabel = (items, judul, warna) => {
        if (items.length === 0) return "";
        const head = isLoggedIn
            ? `<tr><th class="py-4 px-6 text-[10px] font-bold uppercase tracking-widest text-gray-400 w-[30%]">Informasi Unit</th><th class="py-4 px-6 text-[10px] font-bold uppercase tracking-widest text-gray-400 w-[45%]">Alamat Lengkap</th><th class="py-4 px-6 text-[10px] font-bold uppercase tracking-widest text-gray-400 w-[25%]">Penanggung Jawab</th></tr>`
            : `<tr><th class="py-4 px-6 text-[10px] font-bold uppercase tracking-widest text-gray-400 w-[15%] text-center">Kecamatan</th><th class="py-4 px-6 text-[10px] font-bold uppercase tracking-widest text-gray-400 w-[15%] text-center">Desa</th><th class="py-4 px-6 text-[10px] font-bold uppercase tracking-widest text-gray-400 w-[25%]">Nama Unit</th><th class="py-4 px-6 text-[10px] font-bold uppercase tracking-widest text-gray-400 w-[25%] text-center">Alamat Lengkap</th><th class="py-4 px-6 text-[10px] font-bold uppercase tracking-widest text-gray-400 w-[20%] text-center">PJ / Pengelola</th></tr>`;
        ...
        items.forEach(u => {
            ...
            if (!isLoggedIn) {
                rows += `<tr class="hover:bg-gray-50/50 border-b border-gray-100 last:border-0 transition-colors">
                    ...
                    <td class="py-4 px-6 text-xs font-black text-[#00ac69] text-center">${d}</td>
                    <td class="py-4 px-6 text-sm font-bold text-[#1f2d41]">${n}</td>
                    <td class="py-4 px-6 text-xs text-gray-500 italic max-w-xs truncate" title="${a}">${a}</td>
                    <td class="py-4 px-6 text-xs font-bold text-gray-400 text-center uppercase tracking-tighter">${pj}</td>
                </tr>`;
            } else {
                let info = "";
                if (u.topik === 'kopdes') {
                    const sk = (u.sk_ahu || "").toString();
                    info += `<div class="mt-1 flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-blue-500"></span><span class="text-[10px] font-black text-blue-600 uppercase tracking-tighter">${sk.toUpperCase().includes("SK AHU") ? sk : "SK AHU: " + sk}</span></div>`;
                }
                rows += `<tr class="hover:bg-gray-50/50 border-b border-gray-100 last:border-0 transition-colors">
                    <td class="py-5 px-6">
                    ...
                    <td class="py-5 px-6">
                        <div class="text-[12px] leading-relaxed text-gray-500 max-w-sm">${a}</div>
                    </td>
                    <td class="py-5 px-6">
                        <div class="text-xs font-medium text-gray-600">${pj}</div>
                    </td>
                </tr>`;
            }
        });
        ...
        return `<div class="mb-8 last:mb-0">
            ...
            <div class="overflow-hidden rounded-2xl border border-gray-100 shadow-sm">
                <table class="w-full text-left bg-white min-w-[500px]">
    ...
    container.innerHTML = buatTabel(list.filter(i => i.topik === 'sppg'), "Data SPPG", "border-[#00ac69]") +
        buatTabel(list.filter(i => i.topik === 'kopdes'), "Data KOPDES", "border-blue-500");
...
function muatDaftarKegiatan(list) {
    ...
    const buatTabelKeg = (items, topik) => {
        ...
        const head = `<tr class="bg-gray-50/50 border-b border-gray-100">
            <th class="py-4 px-6 text-[10px] font-bold uppercase tracking-widest text-gray-400 w-[15%]">Tanggal</th>
            <th class="py-4 px-6 text-[10px] font-bold uppercase tracking-widest text-gray-400 w-[30%]">Deskripsi Kegiatan</th>
            <th class="py-4 px-6 text-[10px] font-bold uppercase tracking-widest text-gray-400 w-[20%]">Wilayah</th>
            <th class="py-4 px-6 text-[10px] font-bold uppercase tracking-widest text-gray-400 w-[20%]">Penanggung Jawab</th>
            <th class="py-4 px-6 text-[10px] font-bold uppercase tracking-widest text-gray-400 w-[15%] text-center">Foto</th>
        </tr>`;
        let rows = '';
        items.forEach(k => {
            const tgl = k.tanggal ? new Date(k.tanggal).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' }) : '-';
            ...
            const pj = k.penanggung_jawab || '-';
            const pihak = k.pihak_luar && k.pihak_luar !== '-' ? `<div class="flex items-center gap-1.5 mt-1.5"><span class="text-[9px] font-bold text-gray-300 uppercase">Pihak:</span> <span class="text-[11px] font-medium text-gray-500">${k.pihak_luar}</span></div>` : '';
            ...
            rows += `<tr class="hover:bg-gray-50/50 border-b border-gray-100 last:border-0 transition-colors">
                ...
                <td class="py-5 px-6"><div class="text-sm font-black text-[#1f2d41]">${nama}</div>${pihak}${ket}</td>
        ...
        return `<div class="mb-8 last:mb-0">
            <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-[3px] mb-4 flex items-center gap-2">
...
function renderChartKegiatan() {
    ...
    // Inisialisasi 20 Kecamatan Nganjuk
    const list20 = [
    ...
    list20.forEach(k => {
        kecMap[k] = { intel: 0, pidum: 0, datun: 0 };
    });

    // Validasi data dan hitung
    dataKegiatan.forEach(k => {
        const namaKec = clean.kec(k.kecamatan || '');
        if (!namaKec) return;

        // Memastikan namaKecamatan terdaftar (misal typo di database)
        if (kecMap[namaKec]) {
            const t = k.topik;
            if (t === 'intel' || t === 'pidum' || t === 'datun') {
                kecMap[namaKec][t]++;
            }
        }
    });
    ...
    const labels = list20;
    const dataIntel = labels.map(k => kecMap[k].intel);
    const dataPidum = labels.map(k => kecMap[k].pidum);
    const dataDatun = labels.map(k => kecMap[k].datun);

    if (chartInstance) chartInstance.destroy();

    const btnIntel = document.querySelector('button[onclick*="toggleChartDataset(0"]');
    const btnPidum = document.querySelector('button[onclick*="toggleChartDataset(1"]');
    const btnDatun = document.querySelector('button[onclick*="toggleChartDataset(2"]');

    if (btnIntel) btnIntel.className = 'px-3 py-1 text-[10px] font-bold rounded-full border transition-all border-emerald-500 bg-emerald-50 text-emerald-600';
    if (btnPidum) btnPidum.className = 'px-3 py-1 text-[10px] font-bold rounded-full border transition-all border-red-500 bg-red-50 text-red-600';
    if (btnDatun) btnDatun.className = 'px-3 py-1 text-[10px] font-bold rounded-full border transition-all border-yellow-500 bg-yellow-50 text-yellow-600';
...
function toggleChartDataset(index, btn, color) {
    ...
    if (!isHidden) {
        btn.className = 'px-3 py-1 text-[10px] font-bold rounded-full border transition-all border-gray-200 bg-gray-50 text-gray-400';
    } else {
        if (color === 'emerald') btn.className = 'px-3 py-1 text-[10px] font-bold rounded-full border transition-all border-emerald-500 bg-emerald-50 text-emerald-600';
        else if (color === 'red') btn.className = 'px-3 py-1 text-[10px] font-bold rounded-full border transition-all border-red-500 bg-red-50 text-red-600';
        else if (color === 'yellow') btn.className = 'px-3 py-1 text-[10px] font-bold rounded-full border transition-all border-yellow-500 bg-yellow-50 text-yellow-600';
    }
}
...
function siramWarnaKePeta() {
    ...
    geojsonLayer.eachLayer(l => {
        ...
        const info = dataWilayah.find(w => {
            if (w.level !== 'kecamatan') return false;
        ...
        const kopdesCount = units.filter(u => u.topik === 'kopdes').length;
        ...
        const tooltipHtml = `<div style="text-align:left; max-width:260px;">
            ...
            <div style="display:flex; gap:8px; font-size:10px; color:#6b7280; justify-content:center;">
                <span>${sppgCount} SPPG</span><span>|</span><span>${kopdesCount} KOPDES</span>
            </div>
            ${kegLines ? '<div style="border-top:1px solid #e5e7eb; margin-top:4px; padding-top:4px;">' + kegLines + '</div>' : ''}
        </div>`;

        l.feature.properties.data_tambahan = info || { nama_wilayah: namaGeo };
        l.bindTooltip(tooltipHtml, { sticky: true, className: 'custom-tooltip' });
    });
}
...
function renderMarkerUnit(list, listKeg = null) {
    ...
    const warnaTopik = { sppg: '#00ac69', kopdes: '#3b82f6', intel: '#10b981', pidum: '#ef4444', datun: '#eab308' };

    list.forEach(u => {
        const d = clean.desa(u.lokasi_desa || u.desa || "");
        const k = clean.kec(u.lokasi_kecamatan || u.kecamatan || "");
        let ref = semuaDataDesa.find(item => clean.desa(item.nama_desa) === d && clean.kec(item.kecamatan) === k) || semuaDataDesa.find(item => clean.kec(item.kecamatan) === k);
        const lat = ref ? ref.lat : -7.6044, lng = ref ? ref.lng : 111.9044;
        const color = warnaTopik[u.topik] || '#00ac69';

        L.circleMarker([lat + (Math.random() - 0.5) * 0.015, lng + (Math.random() - 0.5) * 0.015], {
            radius: 6, fillColor: color, color: "#fff", weight: 2, fillOpacity: 0.9
        }).bindTooltip(`<b>${u.nama_unit || u.nama_kopdes || "-"}</b>`, { direction: 'top' }).addTo(markerUnitsLayer);
    });
    ...
    dataKeg.forEach(kg => {
        ...
        const nama = kg.nama_kegiatan || '-';
        const bidang = kg.topik ? kg.topik.charAt(0).toUpperCase() + kg.topik.slice(1) : '';

        L.circleMarker([lat + (Math.random() - 0.5) * 0.012, lng + (Math.random() - 0.5) * 0.012], {
            radius: 5, fillColor: color, color: color, weight: 1.5, fillOpacity: 0.7, dashArray: '3'
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const payload = {
                ...
                penanggung_jawab: document.getElementById('input-keg-pj').value || '-',
